...
def validate_token(token: str):
    ...
    guest_user = frappe.get_doc('Chatting Profile', {"token": token})

    if guest_user.ip_address != frappe.local.request_ip:
        return [False, {}]

    room = frappe.db.get_value(
        'Chatting Room', {'guest': guest_user.email}, ['name'])
...
def update_room(room, last_message=None, is_read=0, update_modified=True):
    """Update the value of Chatting room doctype with latest real time values
    ...
    if is_read == 1:
        is_read_members = frappe.db.get_value(
            'Chatting Room', room, 'is_read') or ''
        is_read_members += f'{frappe.session.user}, '
        new_values['is_read'] = is_read_members
    else:
        new_values['is_read'] = ''

    frappe.db.set_value('Chatting Room', room, new_values,
                        update_modified=update_modified)


def get_Chatting_settings():
    """Get the Chatting settings
    Returns:
        dict: Dictionary containing Chatting settings.
    """
    Chatting_settings = frappe.get_cached_doc('hatting Settings')
    user_roles = frappe.get_roles()

    allowed_roles = [u.role for u in Chatting_settings.allowed_roles]
    allowed_roles.extend(['System Manager', 'Administrator'])
    result = {
        'enable_Chatting': False
    }

    if frappe.session.user == 'Guest':
        result['enable_Chatting'] = True

    if not Chatting_settings.enable_Chatting or not has_common(allowed_roles, user_roles):
        return result

    Chatting_settings.Chatting_operators = [co.user for co in Chatting_settings.Chatting_operators]

    if Chatting_settings.start_time and Chatting_settings.end_time:
        start_time = datetime.time.fromisoformat(Chatting_settings.start_time)
        end_time = datetime.time.fromisoformat(Chatting_settings.end_time)
        current_time = datetime.datetime.now().time()

        Chatting_status = 'Online' if time_in_range(
            start_time, end_time, current_time) else 'Offline'
    else:
        Chatting_status = 'Online'

    result['enable_Chatting'] = True
    result['Chatting_status'] = Chatting_status
    return result
...
def display_warning():
    ...
    """
    message = 'The Chatting application in frappe is deprecated and will be removed in the future release. So please use this one only.'
...
def get_user_settings():
    ...
    Returns:
        dict: user settings
    """
    if frappe.db.exists('Chatting User Settings', frappe.session.user):
        user_doc = frappe.db.get_value('Chatting User Settings', frappe.session.user, [
...
def get_room_detail(room):
    ...
    room_detail = frappe.db.get_value(
        'Chatting Room', room, ['members', 'type', 'guest'], as_dict=True)
    return room_detail
