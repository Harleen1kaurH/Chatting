...
export default class chattingSpace {
  constructor(opts) {
  ...
  setup_header() {
    ...
    const header_html = `
          ...
          this.profile.is_admin === true
            ? `<span class='chatting-back-button' title='${__('Go Back')}' >
  ...
  handle_send_message(attachment) {
    ...
    if (
      this.profile.is_admin === true &&
      frappe.chatting.settings.user.enable_message_tone === 1
    ) {
      frappe.utils.play_sound('chatting-message-send');
    }

    this.$chatting_space_container.append(
  ...
  receive_message(res, time) {
    ...
    this.$chatting_space_container.append(
      this.make_message(res.content, time, chatting_type, res.user)
    );
    scroll_to_bottom(this.$chatting_space_container);
  }
