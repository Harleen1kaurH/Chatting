(() => {
  ...
  // ../chatting/chatting/public/js/components/chatting_bubble.js
  var chattingBubble = class {
    constructor(parent) {
      this.parent = parent;
      this.setup();
    }
    setup() {
      ...
      this.open_title = this.parent.is_admin ? __("Show chattings") : __("chatting With Us");
  ...
  // ../chatting/chatting/public/js/components/chatting_utils.js
  ...
  async function get_messages(room, email) {
    const res = await frappe.call({
      method: "chatting.api.message.get_all",
      args: {
        room,
        email
      }
  ...
  async function send_message(content, user, room, email) {
    try {
      await frappe.call({
        method: "chatting.api.message.send",
        args: {
          content,
          user,
          room,
  ...
  async function get_settings(token) {
    const res = await frappe.call({
      type: "GET",
      method: "chatting.api.config.settings",
      args: {
        token
      }
    });
  ...
  async function mark_message_read(room) {
    try {
      await frappe.call({
        method: "chatting.api.message.mark_as_read",
        args: {
          room
        }
      });
  ...
  async function create_guest({ email, full_name, message }) {
    const res = await frappe.call({
      method: "chatting.api.user.get_guest_room",
      args: {
        email,
        full_name,
        message
  ...
  async function set_typing(room, user, is_typing, is_guest) {
    try {
      await frappe.call({
        method: "chatting.api.message.set_typing",
        args: {
          room,
          user,
          is_typing,
  ...
  async function create_private_room(room_name, users, type) {
    await frappe.call({
      method: "chatting.api.room.create_private",
      args: {
        room_name,
        users,
        type
  ...
  async function set_user_settings(settings) {
    await frappe.call({
      method: "chatting.api.config.user_settings",
      args: {
        settings
      }
    });
  ...
  function set_notification_count(type) {
    const current_count = frappe.chatting.settings.unread_count;
    if (type === "increment") {
      $("#chatting-notification-count").text(current_count + 1);
      frappe.chatting.settings.unread_count += 1;
    } else {
      if (current_count - 1 === 0) {
        $("#chatting-notification-count").text("");
      } else {
        $("#chatting-notification-count").text(current_count - 1);
      }
      frappe.chatting.settings.unread_count -= 1;
  ...
  // ../chatting/chatting/public/js/components/chatting_space.js
  var chattingSpace = class {
    constructor(opts) {
    ...
    setup() {
      this.$chatting_space = $(document.createElement("div"));
      this.typing = false;
      this.$chatting_space.addClass("chatting-space");
      this.setup_header();
    ...
    setup_header() {
      ...
      const header_html = `
			<div class='chatting-header'>
				${this.profile.is_admin === true ? `<span class='chatting-back-button' title='${__("Go Back")}' >
				...
				<div class='chatting-profile-info'>
					...
					<div class='chatting-profile-status'>${__("Typing...")}</div>
				</div>
			</div>
		`;
      this.$chatting_space.append(header_html);
    }
    ...
    setup_messages(messages_list) {
      this.$chatting_space_container = $(document.createElement("div"));
      this.$chatting_space_container.addClass("chatting-space-container");
      this.make_messages_html(messages_list);
      this.$chatting_space_container.html(this.message_html);
      this.$chatting_space.append(this.$chatting_space_container);
    }
    ...
    setup_actions() {
      this.$chatting_actions = $(document.createElement("div"));
      this.$chatting_actions.addClass("chatting-space-actions");
      const chatting_actions_html = `
			<span class='open-attach-items'>
				${frappe.utils.icon("attachment", "lg")}
			</span>
			<input type='file' id='chatting-file-uploader' 
		...
		`;
      this.$chatting_actions.html(chatting_actions_html);
      this.$chatting_space.append(this.$chatting_actions);
    }
    ...
    upload_file(file) {
      ...
      return new Promise((resolve, reject) => {
        ...
        form_data.append("doctype", "chatting Room");
    ...
    setup_events() {
      ...
      $(".chatting-back-button").on("click", function() {
        me.chatting_list.render_messages();
        me.chatting_list.render();
      });
      $(".open-attach-items").on("click", function() {
        $("#chatting-file-uploader").click();
      });
      $("#chatting-file-uploader").on("change", function() {
      ...
      if (res.user != this.profile.user_email) {
        if (this.profile.is_admin === true && res.is_guest === "true" || this.profile.is_admin === false || this.profile.room_type === "Group" || this.profile.room_type === "Direct") {
          if (res.is_typing === "false") {
            $(".chatting-profile-status").css("visibility", "hidden");
          } else {
            $(".chatting-profile-status").css("visibility", "visible");
    ...
    make_message(content, time, type, name) {
      ...
      if (content.startsWith("/files/") && file_name !== "") {
        ...
        if (is_image(file_name)) {
          ...
          $url.attr({ src: content }).addClass("img-responsive chatting-image");
    ...
    handle_send_message(attachment) {
      ...
      if (this.profile.is_admin === true && frappe.chatting.settings.user.enable_message_tone === 1) {
        frappe.utils.play_sound("chatting-message-send");
      }
      this.$chatting_space_container.append(this.make_message(content, get_time(), "recipient", this.profile.user));
      $type_message.val("");
      scroll_to_bottom(this.$chatting_space_container);
      send_message(content, this.profile.user, this.profile.room, this.profile.user_email);
    }
    receive_message(res, time) {
      let chatting_type = "sender";
      if (res.sender_email === this.profile.user_email) {
        return;
      }
      if (this.profile.is_admin === true && $(".chatting-element").is(":visible") && frappe.chatting.settings.user.enable_message_tone === 1) {
        frappe.utils.play_sound("chatting-message-receive");
      }
      if (this.profile.room_type === "Guest") {
        if (this.profile.is_admin === true && res.user !== "Guest") {
          chatting_type = "recipient";
        }
      }
      this.$chatting_space_container.append(this.make_message(res.content, time, chatting_type, res.user));
      scroll_to_bottom(this.$chatting_space_container);
    }
    render() {
      this.$wrapper.html(this.$chatting_space);
      this.setup_events();
      scroll_to_bottom(this.$chatting_space_container);
    }
  };

  // ../chatting/chatting/public/js/components/chatting_room.js
  var chattingRoom = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
  ...
  // ../chatting/chatting/public/js/components/chatting_add_room.js
  var chattingAddRoom = class {
    constructor(opts) {
      this.user = opts.user;
  ...
  // ../chatting/chatting/public/js/components/chatting_user_settings.js
  var chattingUserSettings = class {
    constructor() {
      this.setup();
    }
    async setup() {
        ...
        action: {
          primary: {
            ...
            onsubmit: (values) => {
              ...
              frappe.chatting.settings.user.enable_message_tone = values.enable_message_tone;
              frappe.chatting.settings.user.enable_notifications = values.enable_notifications;
  ...
  // ../chatting/chatting/public/js/components/chatting_list.js
  var chattingList = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
      this.user = opts.user;
      this.user_email = opts.user_email;
    ...
    setup_socketio() {
      ...
      frappe.realtime.on("latest_chatting_updates", function(res) {
        const chatting_room_item = me.chatting_rooms.find((element) => element[0] === res.room);
        if (typeof chatting_room_item === "undefined") {
          return;
        }
        if (!$(".chatting-element").is(":visible") && frappe.chatting.settings.user.enable_notifications === 1) {
  ...
  // ../chatting/chatting/public/js/components/chatting_form.js
  var chattingForm = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
      this.profile = opts.profile;
      this.setup();
  ...
  // ../chatting/chatting/public/js/components/chatting_welcome_screen.js
  var chattingWelcome = class {
    constructor(opts) {
      this.$wrapper = opts.$wrapper;
      this.profile = opts.profile;
      this.setup();
    ...
    setup() {
      this.$chatting_welcome_screen = $(document.createElement("div")).addClass("chatting-welcome");
      const welcome_html = `
			<div class='chatting-welcome-header'>
		...
		`;
      const status_text = this.profile.chatting_status === "Online" ? __("We are online") : __("We are offline");
      const reason_text = this.profile.chatting_status === "Online" ? __("Typically replies in a few hours") : __("Just drop a message and we will get back to you soon");
      const bottom_html = `
			<div class='chatting-welcome-footer'>
				...
				<a class='chatting-footer welcome-footer' target='_blank' href='https://frappeframework.com/'>
					${__("\u26A1 Powered by Frappe")}
				</a>
			</div>
		`;
      this.$chatting_welcome_screen.append(welcome_html + bottom_html);
    }
    setup_events() {
      const me = this;
      $("#start-conversation").on("click", function() {
        me.chatting_form = new chattingForm({
          $wrapper: me.$wrapper,
          profile: me.profile
        });
        me.chatting_form.render();
      });
    }
    render() {
      this.$wrapper.html(this.$chatting_welcome_screen);
      this.setup_events();
    }
  };

  // ../chatting/chatting/public/js/chatting.bundle.js
  ...
  frappe.chatting = class {
    ...
    create_app() {
      ...
      this.is_open = false;
      this.$chatting_element = $(document.createElement("div")).addClass("chatting-element").hide();
    ...
    should_close(e) {
      ...
      const navbar = $(".navbar");
      const modal = $(".modal");
      return !chatting_app.is(e.target) && chatting_app.has(e.target).length === 0 && !navbar.is(e.target) && navbar.has(e.target).length === 0 && !modal.is(e.target) && modal.has(e.target).length === 0;
    }
    setup_events() {
      const me = this;
...
//! momentjs.com
//! version : 2.29.4
//# sourceMappingURL=chatting.bundle.CZXXH5J2.js.map
