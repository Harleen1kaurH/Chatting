...
def generate_guest_room(email: str, full_name: str, message: str) -> Tuple[str, str]:
    chat_operators = frappe.get_cached_doc("Chatting Settings").chat_operators or []
...
def get_guest_room(*, email: str, full_name: str, message: str) -> Dict[str, str]:
    ...
    """
    if not frappe.db.exists("Chatting Profile", email):
        room, token = generate_guest_room(email, full_name, message)

    else:
        room = frappe.db.get_value("Chatting Room", {"guest": email}, "name")
        token = frappe.db.get_value("Chatting Profile", email, "token")
