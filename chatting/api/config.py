...
from chatting.utils import validate_token, get_admin_name, get_chatting_settings, get_user_settings
import json
...
def settings(token):
    ...
    config = {**config, **get_chatting_settings()}
...
def user_settings(settings):
    ...
    if not frappe.db.exists('Chatting User Settings', frappe.session.user):
        settings_doc = frappe.get_doc({
            'doctype': 'Chatting User Settings',
        ...
        settings_doc = frappe.get_doc(
            'Chatting User Settings', frappe.session.user)
