...
from chatting.utils import update_room, is_user_allowed_in_room, raise_not_authorized_error
...
def send(content: str, user: str, room: str, email: str):
    ...
    for chat_user in frappe.get_cached_doc("Chatting Room", room).get_members():
...
def mark_as_read(room: str):
    ...
    """
    frappe.enqueue(
        "chatting.utils.update_room", room=room, is_read=1, update_modified=False
    )
