...
def get(email: str) -> List[Dict]:
    ...
    """
    room_doctype = frappe.qb.DocType('Chatting Room')
    ...
    for room in all_rooms:
        ...
        if room['type'] == 'Guest':
            users = frappe.get_cached_doc("Chatting Room", room['name']).users
            if not users:
                users = frappe.get_cached_doc('Chatting Settings').chat_operators
