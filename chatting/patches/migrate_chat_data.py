...
from chatting.utils import get_full_name
...
def migrate_rooms():
    old_rooms_list = frappe.qb.from_(
        'Chatting Room').select('*').run(as_dict=True)

    for room in old_rooms_list:
        room_doc = frappe.get_doc('Chatting Room', room['name'])
        room_users = frappe.get_all('Chatting Room User', filters={
            'parent': room['name']
        },
...
def migrate_messages():
    ...
    for room in old_rooms_list:
        message_list = frappe.db.get_all('Chatting Message', filters={
            'room': room['name']
        }, fields=['content', 'owner', 'name'])

        for message_item in message_list:
            message_doc = frappe.get_doc('Chatting Message', message_item['name'])
